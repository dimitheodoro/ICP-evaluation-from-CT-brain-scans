...
def MultipleInputsResnet101(input_shape, sex_label_shape, age_label_shape, GCS_label_shape, 
                              sex_num_classes=2, age_num_classes=3, GCS_num_classes=3):
    Resnet101, _ = Classifiers.get('resnet101')
    ...
    base_model = Resnet101(input_shape=(128, 128, 123, 3), weights='imagenet',include_top=False)
    base_model = base_model(merge)
    print(f"Shape after Resnet101: {base_model.shape}")
...
if __name__ == '__main__':
    ...
    model = MultipleInputsResnet101(input_shape, sex_label_shape, age_label_shape, GCS_label_shape)
    model.summary()
